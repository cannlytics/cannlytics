...
# TODO: Calculate statistics for Nevada.

# nv_flower = nv_flower.dropna(subset=['date_tested'])
# nv_flower['month'] = nv_flower['date_tested'].dt.to_period('M')
# market_share = nv_flower.groupby(['month', 'lab']).size().unstack().fillna(0)
...
#     2019: 3_090_771,
# }
# nv_flower['year'] = nv_flower['date_tested'].dt.year
# nv_flower['population'] = nv_flower['year'].map(nv_population)
# nv_tests_per_capita = nv_flower.groupby('month').size() / (nv_flower.groupby('month')['population'].first() / 100_000)
...
# nv_flower['total_thc'] = nv_flower['total_thc'].astype(float)
# average_total_thc = nv_flower.groupby('month')['total_thc'].mean()
# fig, ax = plt.subplots(figsize=(13, 8))
...
# ax.scatter(nv_flower['date_tested'], nv_flower['total_thc'], color='royalblue', s=10, alpha=0.5, label='Daily Individual Results')
# ax.set_xlabel('')
...
# nv_flower['total_cbd'] = nv_flower['total_cbd'].astype(float)
# sample = nv_flower.loc[nv_flower['total_cbd'] < 1]
...
# nv_flower['total_terpenes'] = nv_flower['total_terpenes'].astype(float)
# average_total_terpenes = nv_flower.groupby('month')['total_terpenes'].mean()
# fig, ax = plt.subplots(figsize=(13, 8))
...
# ax.scatter(nv_flower['date_tested'], nv_flower['total_terpenes'], color='royalblue', s=10, alpha=0.5, label='Daily Individual Results')
# ax.set_xlabel('')
...
# plt.xticks(rotation=45)
# plt.ylim(0, 4.5)
# plt.xlim(nv_flower['date_tested'].min(), pd.to_datetime('2021-04-01'))
...
# # plt.scatter(nv_flower['total_thc'], nv_flower['total_cbd'])
...
# sample = nv_flower[
#     (nv_flower['total_cannabinoids'] <= 40) &
#     (nv_flower['total_cannabinoids'] > 0) &
#     (nv_flower['total_terpenes'] > 0) &
#     (nv_flower['total_terpenes'] <= 4.5)
# ]
